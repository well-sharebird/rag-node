...
import { Button, Card, CardHeader, CardBody, CardTitle, Input, Badge, Modal } from '@/src/components/enterprise';
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-white">
      ...
      <header className="h-[60px] px-6 bg-white flex items-center justify-between shrink-0 border-b border-[var(--gray-200)]">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            className="text-[13px] rounded-xl bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white"
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left: Skill List */}
        <div className="w-[340px] shrink-0 bg-white border-r border-[var(--gray-200)] flex flex-col">
            ...
            {filteredSkills.map(s => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-full text-left px-4 py-3 hover:bg-[var(--bg-primary)] transition-colors border-b border-[var(--gray-100)]",
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    {s.version_count > 0 && (
                      <span className="text-[11px] text-[var(--text-secondary)]">{s.version_count} versions</span>
                    )}
                  </div>
                </button>
              );
              ...
              <Card>
                <CardBody>
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <h2 className="text-[16px] font-semibold text-[var(--text-primary)]">{selectedSkill.name}</h2>
                      {selectedSkill.description && (
                        <p className="text-[13px] text-[var(--text-secondary)] mt-1">{selectedSkill.description}</p>
                      )}
                    </div>
                    <Button
              ...
              <Card>
                ...
                <CardBody className="p-0">
                  <div className="divide-y divide-[var(--gray-100)] max-h-[300px] overflow-y-auto">
                    {versions.map(v => (
                      <div key={v.id} className="px-5 py-3 flex items-center justify-between hover:bg-[var(--bg-primary)]">
                        <div>
                          <span className="text-[14px] font-mono font-medium text-[var(--text-primary)]">v{v.version}</span>
                          <Badge
                          ...
                          {v.changelog && (
                            <p className="text-[12px] text-[var(--text-secondary)] mt-0.5 truncate max-w-[400px]">{v.changelog}</p>
                          )}
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] text-[var(--text-tertiary)] font-mono">{v.package_hash?.slice(0, 8)}</span>
                          <Button
                            variant="ghost"
                            size="sm"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Card>
                  ...
                  <CardBody className="p-3 space-y-2 max-h-[200px] overflow-y-auto">
                    {tags.map(tg => (
                      <div key={tg.id} className="flex items-center justify-between px-3 py-2 rounded-lg bg-[var(--bg-primary)]">
                        <span className="text-[13px] font-mono font-medium text-[var(--accent)]">{tg.tag_name}</span>
                        <span className="text-[12px] text-[var(--text-secondary)]">→ v{tg.version}</span>
                      </div>
                    ))}
                    {tags.length === 0 && (
                      <div className="text-center text-[12px] text-[var(--text-tertiary)] py-4">{t('skill.noTags')}</div>
                    )}
                  </CardBody>
                </Card>
                ...
                <Card>
                  ...
                  <CardBody className="p-3 text-center text-[12px] text-[var(--text-secondary)] py-4">
                    {t('skill.lockHint')}
                  </CardBody>
                </Card>
              </div>
                    ...
                    <div className="space-y-2">
                      {deps.map((d, i) => (
                        <div key={i} className="flex items-center gap-2 py-1.5">
                          <span className="font-medium text-[var(--accent)]">{d.dep_skill_name}</span>
                          <span className="text-[var(--text-secondary)] text-[12px]">{d.constraint}</span>
                          <ChevronRight className="w-3 h-3 text-[var(--text-tertiary)]" />
                          <span className="font-mono text-[var(--success)] text-[13px]">{d.resolved_version}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-center text-[12px] text-[var(--text-secondary)] py-4">
                      Click <b className="text-[var(--text-primary)]">Deps</b> on a version to view its dependency tree.
                    </div>
                  )}
                </CardBody>
              </Card>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={handleCreate} disabled={!createName.trim()} className="bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white">
              Create
            </Button>
          </>
        }
      >
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[var(--text-secondary)]">Category</label>
            <Select
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={handlePublish} disabled={!publishVersion || !publishFile} className="bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white">
              Publish
            </Button>
          </>
        }
      >
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[var(--text-secondary)]">Version (SemVer)</label>
            <Input
          ...
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[var(--text-secondary)]">Changelog</label>
            <Input
          ...
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[var(--text-secondary)]">Dependencies (JSON)</label>
            <textarea
              value={publishDeps}
              onChange={e => setPublishDeps(e.target.value)}
              placeholder={`[{"dep_skill_name":"other-skill","version_constraint":">=1.0.0"}]`}
              className="w-full text-[13px] border border-[var(--gray-200)] rounded-xl px-3 py-2 h-20 resize-none focus:outline-none focus:ring-2 focus:ring-[var(--accent)]"
            />
            <span className="text-[10px] text-[var(--text-tertiary)]">JSON array, leave empty if no deps</span>
          </div>
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[var(--text-secondary)]">Package File</label>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={handleSetTag} disabled={!tagName || !tagVersion} className="bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white">
              Set Tag
            </Button>
          </>
        }
      >
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[var(--text-secondary)]">Tag Name</label>
            <Input
              value={tagName}
          ...
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[var(--text-secondary)]">Target Version</label>
            <Input
              value={tagVersion}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={handleSetLock} disabled={!lockUser || !lockVersion} className="bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white">
              Lock
            </Button>
          </>
        }
      >
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[var(--text-secondary)]">User ID</label>
            <Input
              value={lockUser}
          ...
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[var(--text-secondary)]">Lock Version</label>
            <Input
              value={lockVersion}
