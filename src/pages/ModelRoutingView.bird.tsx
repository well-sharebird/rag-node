...
import { Card, CardHeader, CardTitle, CardBody, CardFooter, CardDescription } from '../components/bird/Card';
...
export function ModelRoutingView() {
  ...
  const getProviderBadgeColor = (providerId: number) => {
    ...
    if (provider.provider_type === 'self_hosted') return 'bg-purple-500';
    return 'bg-gray-400';
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-[var(--bird-bg-primary)]">
      ...
      <header className="h-[60px] px-6 bg-[var(--bird-sidebar-bg)] flex items-center justify-between shrink-0 border-b border-[var(--bird-sidebar-border)]">
        <div>
          <h1 className="text-[18px] font-semibold text-[var(--bird-text-primary)]">路由配置</h1>
          <p className="text-xs text-[var(--bird-text-tertiary)]">配置模型请求的路由规则和故障转移策略</p>
        </div>
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <GitBranch className="w-16 h-16 text-[var(--bird-text-tertiary)] mb-4" />
              <h3 className="text-lg font-semibold text-[var(--bird-text-primary)]">暂无路由规则</h3>
              <p className="text-[var(--bird-text-tertiary)] text-sm mt-2">创建第一个路由规则开始使用</p>
            </div>
          ) : (
              ...
              <Card key={rule.id} className="overflow-hidden">
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center bg-[var(--bird-primary-50)] text-[var(--bird-primary)]">
                ...
                <CardBody>
                  {rule.description && (
                    <p className="text-sm text-[var(--bird-text-secondary)] mb-3">{rule.description}</p>
                  )}
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                    <div className="flex items-center gap-2">
                      <Activity className="w-3.5 h-3.5 text-[var(--bird-text-tertiary)]" />
                      <span className="text-[var(--bird-text-secondary)]">流量权重：{rule.traffic_weight}%</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock className="w-3.5 h-3.5 text-[var(--bird-text-tertiary)]" />
                      <span className="text-[var(--bird-text-secondary)]">超时：{rule.timeout_ms}ms</span>
                    </div>
                    {rule.retry_enabled && (
                      <div className="flex items-center gap-2">
                        <RefreshCw className="w-3.5 h-3.5 text-[var(--bird-text-tertiary)]" />
                        <span className="text-[var(--bird-text-secondary)]">重试：{rule.retry_max_attempts}次</span>
                      </div>
                    )}
                    {rule.failover_enabled && rule.failover_provider_id && (
                      <div className="flex items-center gap-2">
                        <Shield className="w-3.5 h-3.5 text-[var(--bird-text-tertiary)]" />
                        <span className="text-[var(--bird-text-secondary)]">
                ...
                <CardFooter>
                  <div className="flex items-center justify-between w-full">
                    <label className="flex items-center gap-2 text-sm text-[var(--bird-text-secondary)] cursor-pointer">
                      <input
                        type="checkbox"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-[var(--bird-text-tertiary)]">主供应商</span>
                    </div>
                  </div>
                </CardFooter>
              </Card>
  ...
  return (
    <ScrollArea className="max-h-[550px]">
      <div className="space-y-5 p-1">
        {/* 基本信息 */}
        <div>
          <h4 className="text-sm font-medium text-[var(--bird-text-primary)] mb-3">基本信息</h4>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium text-[var(--bird-text-secondary)] mb-1 block">
                规则名称 *
              </label>
              <input
                type="text"
                className={cn("bird-input w-full", errors.name && "border-red-500")}
                value={formData.name}
          ...
          <div className="mt-3">
            ...
            <textarea
              className="bird-input w-full min-h-[60px]"
        ...
        {/* 供应商配置 */}
        <div className="border-t border-[var(--bird-neutral-200)] pt-4">
          <h4 className="text-sm font-medium text-[var(--bird-text-primary)] mb-3">供应商配置</h4>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium text-[var(--bird-text-secondary)] mb-1 block">
                主供应商 *
            ...
            <div>
              ...
              <input
                ...
                placeholder="100"
              />
              <p className="text-xs text-[var(--bird-text-tertiary)] mt-1">数字越小优先级越高</p>
            </div>
          </div>
          <div className="mt-3">
            <label className="text-sm font-medium text-[var(--bird-text-secondary)] mb-1 block">
            ...
            <input
              ...
              className="w-full"
            />
            <div className="flex justify-between text-xs text-[var(--bird-text-tertiary)] mt-1">
              <span>0%</span>
              <span>50%</span>
              <span>100%</span>
            </div>
        ...
        <div className="border-t border-[var(--bird-neutral-200)] pt-4">
          <div className="flex items-center gap-2 mb-3">
            <input
              ...
              className="rounded border-gray-300"
            />
            <label htmlFor="failover_enabled" className="text-sm font-medium text-[var(--bird-text-secondary)]">
              启用故障转移
            </label>
          </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="3"
                  />
                  <p className="text-xs text-[var(--bird-text-tertiary)] mt-1">连续失败次数</p>
                </div>
              </div>
              <div>
                <label className="text-sm font-medium text-[var(--bird-text-secondary)] mb-1 block">
        ...
        {/* 超时和重试配置 */}
        <div className="border-t border-[var(--bird-neutral-200)] pt-4">
          <h4 className="text-sm font-medium text-[var(--bird-text-primary)] mb-3">超时和重试</h4>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium text-[var(--bird-text-secondary)] mb-1 block">
                超时时间 (ms)
          ...
          <div className="mt-3">
            <label className="flex items-center gap-2 text-sm text-[var(--bird-text-secondary)]">
              <input
                type="checkbox"
