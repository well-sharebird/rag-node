...
export function EvaluationPageBird() {
  const { t } = useI18n();
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-[#f9fafb]">
      ...
      <header className="h-[60px] px-6 bg-white flex items-center justify-between shrink-0 border-b border-[#e5e7eb]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[18px] font-semibold text-[#111827]">RAG 评估</h1>
            <p className="text-[12px] text-[#6b7280]">评估检索增强生成质量</p>
          </div>
        </div>
        <Select
          value={selectedKb}
      ...
      {/* Tabs */}
      <div className="px-6 py-3 bg-white border-b border-[#e5e7eb] flex items-center gap-1">
        <button
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {activeTab === 'runs' && (
            <Button
              ...
              disabled={!selectedKb}
              className="rounded-xl bg-[#7c3aed] hover:bg-[#6d28d9] text-white disabled:opacity-50"
        ...
        {!selectedKb ? (
                ...
                <CardBody>
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-[12px] font-medium text-[#4b5563]">问题</label>
                      <textarea
                        value={newQuestion}
                        onChange={(e) => setNewQuestion(e.target.value)}
                        placeholder="输入测试问题..."
                        className="w-full px-3 py-2 text-[13px] rounded-xl border border-[#e5e7eb] focus:outline-none focus:ring-2 focus:ring-[#7c3aed] resize-none"
                        rows={3}
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[12px] font-medium text-[#4b5563]">期望答案</label>
                      <textarea
                        value={newAnswer}
                    ...
                    <div className="space-y-2">
                      <label className="text-[12px] font-medium text-[#4b5563]">难度</label>
                      <Select
                        value={newDifficulty}
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                      ...
                      <Button onClick={handleCreateSample} className="bg-[#7c3aed] hover:bg-[#6d28d9] text-white">
                        创建
                      </Button>
                    </div>
                  </div>
            ...
            {samples.length === 0 ? (
              <Card>
                <CardBody className="py-12 text-center">
                  ...
                  <p className="text-[14px] text-[#6b7280]">暂无 Golden Samples</p>
                  <p className="text-[12px] text-[#9ca3af] mt-1">创建测试样本来评估 RAG 质量</p>
                </CardBody>
              </Card>
            ) : (
              samples.map((sample) => {
                    ...
                    <CardBody>
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          ...
                          <h4 className="text-[14px] font-medium text-[#111827] mb-2">{sample.question}</h4>
                          <div className="text-[13px] text-[#6b7280] bg-[#f9fafb] rounded-xl p-3">
                            <span className="text-[#9ca3af]">期望答案：</span>
          ...
          <div className="max-w-4xl mx-auto space-y-4">
            ...
            {runs.length === 0 ? (
              <Card>
                <CardBody className="py-12 text-center">
                  <BarChart3 className="w-12 h-12 mx-auto mb-4 text-[#d1d5db]" />
                  <p className="text-[14px] text-[#6b7280]">暂无评估运行</p>
                  <p className="text-[12px] text-[#9ca3af] mt-1">点击"运行评估"开始质量评估</p>
                </CardBody>
              </Card>
            ) : (
              runs.map((run) => {
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            ...
                            run.status === 'completed' ? "bg-[#d1fae5] text-[#10b981]" :
                            run.status === 'running' ? "bg-[#dbeafe] text-[#3b82f6]" :
                            run.status === 'failed' ? "bg-[#fee2e2] text-[#ef4444]" :
                            "bg-[#f3f4f6] text-[#9ca3af]"
                          )}>
                            {statusBadge.icon}
                          </div>
                          <div>
                            <h4 className="text-[14px] font-medium text-[#111827]">{run.name}</h4>
                            <div className="flex items-center gap-3 mt-1">
                              <span className="text-[12px] text-[#6b7280] capitalize">{run.status}</span>
                              {run.total_samples && (
                                <span className="text-[12px] text-[#6b7280]">{run.total_samples} 样本</span>
                              )}
                              {run.avg_score && (
