  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-[var(--bird-bg-primary)]">
      ...
      <header className="h-[60px] px-6 bg-[var(--bird-sidebar-bg)] flex items-center justify-between shrink-0 border-b border-[var(--bird-sidebar-border)]">
        <div>
          <h1 className="text-[18px] font-semibold text-[var(--bird-text-primary)]">模型网关</h1>
          <p className="text-xs text-[var(--bird-text-tertiary)]">统一管理 LLM 供应商和路由配置</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {loading ? (
            ...
            <div className="col-span-full flex flex-col items-center justify-center py-20 text-center">
              <Server className="w-16 h-16 text-[var(--bird-text-tertiary)] mb-4" />
              <h3 className="text-lg font-semibold text-[var(--bird-text-primary)]">暂无供应商配置</h3>
              <p className="text-[var(--bird-text-tertiary)] text-sm mt-2">添加第一个 LLM 供应商开始使用</p>
            </div>
          ) : (
            filteredProviders.map((provider) => (
              <Card key={provider.id} hover className="overflow-hidden">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        ...
                        provider.status === 'active' ? 'bg-[var(--bird-success-bg)] text-[var(--bird-success)]' :
                        provider.status === 'error' ? 'bg-[var(--bird-error-bg)] text-[var(--bird-error)]' :
                        'bg-[var(--bird-neutral-100)] text-[var(--bird-text-tertiary)]'
                      )}>
                ...
                <CardBody>
                  <div className="text-xs text-[var(--bird-text-secondary)] mb-3">
                    <code className="px-2 py-1 bg-[var(--bird-neutral-100)] rounded-lg break-all">
                      {provider.base_url}
                    </code>
                  </div>
                  {provider.description && (
                    <p className="text-xs text-[var(--bird-text-tertiary)] line-clamp-2 mb-3">{provider.description}</p>
                  )}
                  {(provider.cost_input !== undefined || provider.cost_output !== undefined) && (
                    <div className="flex items-center gap-2 text-xs mb-3">
                      <DollarSign className="w-3 h-3 text-[var(--bird-text-tertiary)]" />
                      <span className="text-[var(--bird-text-tertiary)]">
                  ...
                  {provider.rate_limit_enabled && (
                    <div className="flex items-center gap-2 text-xs">
                      <Zap className="w-3 h-3 text-[var(--bird-text-tertiary)]" />
                      <span className="text-[var(--bird-text-tertiary)]">
                ...
                <div className="px-6 py-3 border-t border-[var(--bird-neutral-100)] flex items-center justify-between">
                  <label className="flex items-center gap-2 text-xs text-[var(--bird-text-secondary)] cursor-pointer">
                    <input
                      type="checkbox"
                      checked={provider.is_enabled}
                      onChange={() => handleToggleEnabled(provider.id, provider.is_enabled)}
                      className="bird-switch"
                    />
                    启用
                  </label>
                  {!provider.is_default && (
  ...
  return (
    <div className="space-y-4">
      ...
      {!isEdit && presetProviders && (
          <div>
            <label className="text-sm font-medium text-[var(--bird-text-secondary)] mb-2 block">
              快速选择
            </label>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    formData.code === p.code
                      ? "bg-[var(--bird-primary-50)] border-[var(--bird-primary)] text-[var(--bird-primary)]"
                      : "border-[var(--bird-neutral-200)] hover:bg-[var(--bird-neutral-50)]"
                  )}
                >
                  {p.name}
                </button>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            type="text"
            className={cn("bird-input w-full", errors.code && "border-red-500")}
            value={formData.code}
      ...
      <div className="border-t border-[var(--bird-neutral-200)] pt-4">
        <h4 className="text-sm font-medium text-[var(--bird-text-primary)] mb-3">API 配置</h4>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input
              type="text"
              className={cn("bird-input w-full", errors.base_url && "border-red-500")}
      ...
      <div className="border-t border-[var(--bird-neutral-200)] pt-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          />
          <label htmlFor="rate_limit_enabled" className="text-sm font-medium text-[var(--bird-text-secondary)]">
            启用速率限制
          </label>
        </div>
      ...
      {/* 成本配置 */}
      <div className="border-t border-[var(--bird-neutral-200)] pt-4">
        <h4 className="text-sm font-medium text-[var(--bird-text-secondary)] mb-3">成本配置 (每 1K tokens)</h4>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-sm font-medium text-[var(--bird-text-secondary)] mb-1 block">
              输入成本 (美元)
      ...
      {/* 开关选项 */}
      <div className="border-t border-[var(--bird-neutral-200)] pt-4 space-y-2">
