...
import { Button, Card, CardHeader, CardBody, CardTitle, Badge, Input, Modal } from '@/src/components/bird';
import { Select } from '@/src/components/bird/Select';
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/src/components/bird/Tabs';
import {
...
export function PromptTemplateDetailBird({ templateName, onBack }: PromptTemplateDetailProps) {
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-[#f9fafb]">
      ...
      <header className="h-[60px] px-6 bg-white flex items-center justify-between shrink-0 border-b border-[#e5e7eb]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[18px] font-semibold text-[#111827]">{template?.name}</h1>
            ...
            {versions.length === 0 ? (
              <Card>
                <CardBody className="py-12 text-center">
                  <GitBranch className="w-12 h-12 mx-auto mb-4 text-[#d1d5db]" />
                  <p className="text-[14px] text-[#6b7280]">{language === 'zh' ? '暂无版本' : 'No versions'}</p>
                </CardBody>
              </Card>
            ) : (
              versions.map((v) => (
                  ...
                  <CardBody>
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="text-[13px] text-[#6b7280] mb-3 font-mono bg-[#f9fafb] p-3 rounded-lg max-h-32 overflow-y-auto">
        ...
        {activeTab === 'test-cases' && (
          <div className="space-y-4">
            {testCases.length === 0 ? (
              <Card>
                <CardBody className="py-12 text-center">
                  <AlertCircle className="w-12 h-12 mx-auto mb-4 text-[#d1d5db]" />
                  <p className="text-[14px] text-[#6b7280]">{language === 'zh' ? '暂无测试用例' : 'No test cases'}</p>
                </CardBody>
              </Card>
            ) : (
              testCases.map((tc) => (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-[12px] text-[#9ca3af]">ID: {tc.id}</span>
                        </div>
                        <div className="text-[13px] text-[#6b7280] mt-2">
                          <span className="text-[#9ca3af] block mb-1">{language === 'zh' ? '输入上下文：' : 'Input Context: '}</span>
                          <pre className="text-[12px] bg-[#f9fafb] p-2 rounded-lg overflow-auto max-w-md">
                            {JSON.stringify(tc.input_context, null, 2)}
                          </pre>
                        </div>
                        {tc.expected_output && (
                          <div className="text-[13px] text-[#6b7280] mt-2">
                            <span className="text-[#9ca3af] block mb-1">{language === 'zh' ? '期望输出：' : 'Expected Output: '}</span>
                            <p className="text-[12px] bg-[#f9fafb] p-2 rounded-lg max-w-md">{tc.expected_output}</p>
                          </div>
                        )}
                        {tc.expected_behavior && (
                          <div className="text-[13px] text-[#6b7280] mt-2">
                            <span className="text-[#9ca3af] block mb-1">{language === 'zh' ? '期望行为：' : 'Expected Behavior: '}</span>
                            <p className="text-[12px] bg-[#f9fafb] p-2 rounded-lg max-w-md">{tc.expected_behavior}</p>
                          </div>
                        )}
                        <div className="flex items-center gap-2 mt-2">
                          <span className="text-[12px] text-[#9ca3af]">{language === 'zh' ? '优先级：' : 'Priority: '}</span>
                          <div className="flex">
              ...
              <CardBody>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-[15px] font-semibold text-[#111827]">{language === 'zh' ? '评估报告' : 'Evaluation Report'}</h3>
                  </div>
                  <Button
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-4 gap-4">
                      <div className="p-4 bg-[#f9fafb] rounded-xl text-center">
                        <div className="text-[12px] text-[#9ca3af] mb-1">{language === 'zh' ? '平均分' : 'Avg Score'}</div>
                        <div className="text-[24px] font-semibold text-[#111827]">{evalReport.avg_score?.toFixed(1) || '0'}</div>
                      </div>
                    ...
                    {evalReport.detailed_results && evalReport.detailed_results.length > 0 && (
                      <div className="border border-[#e5e7eb] rounded-xl">
                        <div className="p-3 border-b border-[#e5e7eb]">
                          <h4 className="text-[14px] font-semibold text-[#111827]">{language === 'zh' ? '详细结果' : 'Detailed Results'}</h4>
                        </div>
                        <div className="divide-y divide-[#e5e7eb]">
                          {evalReport.detailed_results.map((result, index) => (
                            <div key={index} className="p-3">
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-[12px] text-[#9ca3af]">{language === 'zh' ? '用例' : 'Case'} #{result.case_id}</span>
                              ...
                              {result.reasoning && (
                                <p className="text-[12px] text-[#6b7280] mt-2 line-clamp-2">{result.reasoning}</p>
                              )}
                            </div>
                          ))}
                        </div>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={handleCreateVersion} disabled={creating} className="bg-[#7c3aed] hover:bg-[#6d28d9] text-white">
      ...
      >
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[#4b5563]">{language === 'zh' ? '版本号' : 'Version'}</label>
            <Input
              value={newVersion}
          ...
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[#4b5563]">{language === 'zh' ? '提示词内容' : 'Prompt Content'}</label>
            <textarea
              value={newContent}
              onChange={(e) => setNewContent(e.target.value)}
              className="w-full min-h-[200px] p-3 text-[13px] border border-[#e5e7eb] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7c3aed]"
              placeholder={language === 'zh' ? '输入提示词内容...' : 'Enter prompt content...'}
            />
          </div>
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[#4b5563]">{language === 'zh' ? '更新说明' : 'Changelog'}</label>
            <Input
              value={newChangelog}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={handleSetTag} className="bg-[#7c3aed] hover:bg-[#6d28d9] text-white">{language === 'zh' ? '设置' : 'Set'}</Button>
          </>
        }
      >
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[#4b5563]">{language === 'zh' ? '标签名称' : 'Tag Name'}</label>
            <Select
              value={selectedTag}
          ...
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[#4b5563]">{language === 'zh' ? '关联版本' : 'Select Version'}</label>
            <Select
        ...
        <div className="space-y-4 py-4">
          <div className="grid grid-cols-2 gap-4 max-h-[60vh] overflow-auto">
            <div className="space-y-3">
              <label className="text-[12px] font-medium text-[#4b5563]">{language === 'zh' ? '变量' : 'Variables'}</label>
              {selectedVersion?.variables_schema && selectedVersion.variables_schema.length > 0 ? (
                selectedVersion.variables_schema.map((v) => (
                  <div key={v.name}>
                    <label className="text-[11px] text-[#9ca3af] block mb-1">{v.name}</label>
                    <Input
                ...
                <p className="text-[12px] text-[#9ca3af]">{language === 'zh' ? '无变量' : 'No variables'}</p>
              )}
            </div>
            <div>
              <label className="text-[12px] font-medium text-[#4b5563] mb-2 block">{language === 'zh' ? '渲染结果' : 'Rendered Result'}</label>
              <pre className="text-[12px] bg-[#f9fafb] p-3 rounded-xl h-[200px] overflow-auto whitespace-pre-wrap text-[#374151]">
                {renderedContent || (language === 'zh' ? '点击渲染查看结果' : 'Click Render to see result')}
              </pre>
            </div>
          </div>
          <Button onClick={handleRender} className="w-full bg-[#7c3aed] hover:bg-[#6d28d9] text-white">
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={handleEval} disabled={evaluating || !candidateVersion} className="bg-[#7c3aed] hover:bg-[#6d28d9] text-white">
      ...
      >
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <label className="text-[12px] font-medium text-[#4b5563]">{language === 'zh' ? '选择候选版本' : 'Select Candidate Version'}</label>
            <Select
