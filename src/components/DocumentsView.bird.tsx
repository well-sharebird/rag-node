...
export function DocumentsViewBird() {
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-[#f9fafb]">
      ...
      <header className="h-[60px] px-6 bg-white flex items-center justify-between shrink-0 border-b border-[#e5e7eb]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[18px] font-semibold text-[#111827]">{t('doc.title')}</h1>
            <p className="text-[12px] text-[#6b7280]">{t('doc.desc')}</p>
          </div>
        </div>
      ...
      <div
        ...
        className={cn(
          ...
          isDragging ? "border-[#7c3aed] bg-[#f5f3ff]" : "border-[#e5e7eb] bg-white hover:border-[#a78bfa]"
        )}
      >
        <UploadCloud className="w-8 h-8 mx-auto mb-2 text-[#9ca3af]" />
        <p className="text-[13px] text-[#6b7280]">
          {t('doc.dragDrop')} <span className="text-[#7c3aed]">{t('doc.orClickToUpload')}</span>
        </p>
      </div>
                ...
                <TableBody>
                  {filteredDocs.map(doc => (
                    <TableRow key={doc.id} className={doc.status === 'failed' ? 'bg-red-50' : ''}>
                      ...
                      <TableCell className="text-[#6b7280]">{getKbName(doc.kbId)}</TableCell>
                      <TableCell>
                        <select
                          value={doc.category || ''}
                          onChange={(e) => handleSetCategory(doc.id, e.target.value)}
                          className="bird-input w-[120px]"
                        >
                      ...
                      <TableCell className="text-[#6b7280]">{formatBytes(doc.size)}</TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
      ...
      >
        <div className="space-y-4 py-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <span className="text-[12px] text-[#6b7280]">知识库</span>
              <p className="text-[14px] font-medium">{getKbName(previewDoc.kbId)}</p>
            </div>
            <div>
              <span className="text-[12px] text-[#6b7280]">大小</span>
              <p className="text-[14px] font-medium">{formatBytes(previewDoc.size)}</p>
            </div>
            <div>
              <span className="text-[12px] text-[#6b7280]">状态</span>
          ...
          {previewDoc.content && (
            <div className="border-t border-[#e5e7eb] pt-4">
              <span className="text-[12px] text-[#6b7280]">内容预览</span>
              <div className="mt-2 p-4 bg-[#f9fafb] rounded-xl text-[13px] text-[#6b7280] max-h-[300px] overflow-y-auto">
