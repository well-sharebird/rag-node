...
@pytest.mark.asyncio
async def test_firecracker_availability():
    """测试 Firecracker 可用性"""
    fc_path = "/usr/local/bin/firecracker"
    if not os.path.exists(fc_path):
        pytest.skip("Firecracker not installed")

    # 如果安装了，验证可以导入
    from packages.agent.sandbox.firecracker import (
        FirecrackerSandboxManager,
        get_firecracker_manager,
    )
...
@pytest.mark.asyncio
async def test_firecracker_execution():
    """测试 Firecracker 代码执行"""
    fc_path = "/usr/local/bin/firecracker"
    if not os.path.exists(fc_path):
        pytest.skip("Firecracker not installed")

    from packages.agent.sandbox.firecracker import execute_code_in_firecracker

    result = await execute_code_in_firecracker(
