...
验证 LangGraph act 节点不裸调用工具，而是经 Harness 工具治理门面执行：
    ...
    - 无 execution_manager 时 fail-closed 拒绝执行（不绕过治理）
"""
import pytest
...
class TestActNodeGovernance:
    ...
    @pytest.mark.asyncio
    async def test_act_node_fails_closed_without_manager(self):
        @tool
        ...
        node = create_act_node([ai_read_tool], None, None)  # 无 manager → fail-closed
        out = await node({
        ...
        # 未装配治理门时拒绝执行，不再绕过直达工具
        assert "治理门" in out["tool_results"][0]["result"]
        assert "read:hi" not in out["tool_results"][0]["result"]
