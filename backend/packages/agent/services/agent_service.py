...
from packages.agent.services.agent_graph_factory import StateGraphBuilder, WorkflowState
from packages.rag.services.retrieval_service import search_chunks
...
class AgentFactory:
    ...
    async def _load_basic_tools(self) -> list:
        """加载基础工具"""
        from packages.agent.tools.builtins import get_basic_tools
    ...
    async def _load_mcp_tools(self, mcp_servers: list[str]) -> list:
        """加载 MCP 工具"""
        from langchain_core.tools import tool
        from packages.agent.mcp.tools.kb_tools import (
        ...
        from packages.agent.mcp.tools.model_tools import (
        ...
        from packages.agent.mcp.tools.prompt_tools import (
        ...
        from packages.agent.mcp.tools.agent_tools import (
        ...
        if "kb" in mcp_servers or "all" in mcp_servers:
            ...
            async def mcp_kb_create(name: str, description: str = "") -> str:
                ...
                from packages.rag.schemas.knowledge_base import KBCreateRequest
    ...
    async def _load_skill_tools(self, skills: list[str]) -> list:
        ...
        for skill in skills:
            ...
            if skill_lower in ["kb", "knowledge_base", "knowledge"]:
                from packages.agent.skills.knowledge_base_tools import (
    ...
    def _build_middlewares(self, agent_config: AgentConfig, runtime_config: dict) -> list:
        ...
        if runtime_config.get("plan_mode", False):
            from packages.agent.middlewares.plan_middleware import PlanMiddleware
...
class MetaAgentFactory:
    ...
    async def _get_tools(self) -> list:
        ...
        try:
            from packages.agent.mcp.client import MCPClient
